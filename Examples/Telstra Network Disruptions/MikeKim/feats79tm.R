...
feat4$log_feature = sapply(feat4$log_feature, function(x) unlist(strsplit(x,' '))[2])
feat4$location = sapply(feat4$location, function(x) unlist(strsplit(x,' '))[2])

feat4$prod1 = paste(feat4$log_feature,feat4$location,sep='')
...
write.csv(df_q, "/home/mikeskim/Desktop/kaggle/telstra/features/feats79tm.csv", quote=FALSE, row.names = FALSE)
