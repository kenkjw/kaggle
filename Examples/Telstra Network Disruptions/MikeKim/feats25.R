...
tmp1 <- feat1[ ,.(
  V1 = min(unlist(lapply(strsplit(event_type,' '),function(x) x[2])))
), id]

tmp3 <- feat3[ ,.(
  V3 = min(unlist(lapply(strsplit(resource_type,' '),function(x) x[2])))
), id]

tmp4 <- feat4[ ,.(
  V4 = min(unlist(lapply(strsplit(log_feature,' '),function(x) x[2])))
), id]
...
write.csv(tmp1, "/home/mikeskim/Desktop/kaggle/telstra/features/feats25.csv", quote=FALSE, row.names = FALSE)
